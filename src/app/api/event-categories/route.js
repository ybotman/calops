...
export async function GET(request) {
  ...
  try {
    ...
    const response = await axios.get(`${BE_URL}/api/event-categories?appId=${appId}`);
    ...
    const errorMessage = error.response?.data?.message || error.message || 'Failed to fetch event categories';
