...
export default function LocationsPage() {
  ...
  // Fetch locations on component mount and when app changes
  useEffect(() => {
    const fetchLocations = async () => {
      try {
        setLoading(true);
        console.log(`Fetching locations for AppId: ${currentApp.id}`);
        
        // Fetch all locations in a single request - hardcode appId to 1 since that's what the backend expects
        const response = await axios.get(`/api/locations?appId=1&type=all`);
        ...
        alert(`Failed to fetch locations: ${errorMessage}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDeleteItem = async (item) => {
    try {
      ...
      // Delete the location using the API - hardcode appId to 1
      const deleteResponse = await axios.delete(`/api/locations/${item.type}/${item.id}?appId=1`);
      console.log('Delete response:', deleteResponse.data);
      
      // Force a delay before refreshing to allow server-side propagation
      await new Promise(resolve => setTimeout(resolve, 500));
      
      // Refresh the locations - hardcode appId to 1
      const response = await axios.get(`/api/locations?appId=1&type=all`);
    ...
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Button 
          ...
          color="primary" 
          startIcon={<AddIcon />}
          onClick={() => alert('Add location feature coming soon!')}
        >
          Add Location
        </Button>
      </Box>
